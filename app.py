...
# CSS Personalizzato per estetica Minimalista (CORRETTO)
st.markdown("""
    <style>
    /* Font e Sfondo */
    ...
    /* Header */
    h1 { font-weight: 600; letter-spacing: -1px; color: #1D1D1F; margin-bottom: 20px; }
...
with st.sidebar:
    ...
    t_sel = st.multiselect("Target di riferimento", t_options)
    
    st.markdown("---")
    st.subheader("Percorso Auto")
    if not df_luoghi.empty and len(df_luoghi) >= 2:
        ...
        st.link_button("Calcola percorso", url_p)
...
with t1:
    ...
    if df_map.empty:
        ...
        for _, r in df_map.iterrows():
            ...
            html = f"""
            <div style='font-family: -apple-system, sans-serif; padding: 10px; min-width:160px;'>
                <h4 style='margin:0 0 5px 0; color:#1D1D1F;'>{r['Nome Zona']}</h4>
                <p style='font-size:13px; color:#86868B; margin:0 0 12px 0;'>{r['Target di Riferimento']}</p>
                <a href='{nav}' target='_blank' style='display:block; background:#0071E3; color:white; text-align:center; padding:10px; border-radius:8px; text-decoration:none; font-size:12px; font-weight:600;'>📍 Apri in Mappe</a>
            </div>
            """
...
with t2:
    ...
    with s_add:
        st.write("1. Clicca sulla mappa per catturare le coordinate")
        ...
        if last_c and last_c.get("last_clicked"):
            ...
            st.toast(f"📍 Coordinate selezionate")
        ...
        with st.form("add_f", clear_on_submit=True):
            ...
            t_r = st.text_input("Target (es. Famiglie)")
            ...
            if st.form_submit_button("Salva zona"):
                if name and la_n and lo_n:
                    ...
                    ws.append_row([len(df_luoghi)+1, name, la_n, lo_n, t_n, t_r, ora, note])
                    if t_n not in tipi: get_worksheet("Config").append_row([t_n])
                    st.success("Zona salvata correttamente")
                    st.cache_data.clear()

    with s_edit:
        if not df_luoghi.empty:
            t_edit = st.selectbox("Seleziona luogo", df_luoghi['Nome Zona'].tolist())
            idx = df_luoghi[df_luoghi['Nome Zona'] == t_edit].index[0]
            cur = df_luoghi.iloc[idx]
            with st.form("edit_f"):
                en = st.text_input("Nome", value=cur['Nome Zona'])
                ela = st.text_input("Lat", value=str(cur['Lat']))
                elo = st.text_input("Lon", value=str(cur['Lon']))
                etarg = st.text_input("Target", value=cur['Target di Riferimento'])
                enot = st.text_area("Note", value=cur['Note'])
                if st.form_submit_button("Applica modifiche"):
                    ws = get_worksheet("Luoghi")
                    ws.update(range_name=f'B{idx+2}:H{idx+2}', values=[[en, ela, elo, cur['Tipo di Zona'], etarg, cur['Orari di Affluenza'], enot]])
                    st.success("Dati aggiornati")
                    st.cache_data.clear()

    with s_del:
        if not df_luoghi.empty:
            t_del = st.selectbox("Luogo da rimuovere", df_luoghi['Nome Zona'].tolist())
            if st.button("Elimina definitivamente"):
                ws = get_worksheet("Luoghi")
                ws.delete_rows(ws.find(t_del).row)
                st.toast("Rimosso")
                st.cache_data.clear()
    ...
    with c1:
        if not df_luoghi.empty:
            with st.form("f_form", clear_on_submit=True):
                zf = st.selectbox("Zona", sorted(df_luoghi['Nome Zona'].unique()))
                tl = st.text_input("Team Leader")
                cf = st.text_area("Feedback")
                vf = st.select_slider("Valutazione", options=[1, 2, 3, 4, 5])
                if st.form_submit_button("Invia report"):
                    get_worksheet("Feedback").append_row([len(df_feedback)+1, zf, datetime.now().strftime("%d/%m/%Y %H:%M"), tl, cf, vf])
